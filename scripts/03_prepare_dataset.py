...
Usage (adding a second recording to the same dataset):
    python 03_prepare_dataset.py \
        --audio /mnt/extra/bxm0694/more_audio.m4a \
        --transcript transcript_hi_2.json \
        --out-dir dataset \
        --prefix rec2 \
        --append

--prefix keeps filenames from different source recordings from colliding
(segment IDs restart at 0 for every new transcript). --append adds rows to
the existing metadata.csv/metadata_debug.csv instead of overwriting them —
always pass it after the first recording, or you'll lose earlier clips'
metadata (the wav files themselves are safe either way, only the CSVs are
at risk of being overwritten).

Produces:
    dataset/
      wavs/<prefix>_seg_0000.wav, ...
...
dropping too much data has its own cost — tighten with --min-dur/--max-dur if
you want to match their guidance more strictly.
"""
import argparse
import csv
import os
...
def main():
    ...
    parser.add_argument(
        "--prefix",
        default="rec",
        help="Prefix for output filenames, to avoid collisions across multiple source recordings",
    )
    parser.add_argument(
        "--append",
        action="store_true",
        help="Append to existing metadata.csv/metadata_debug.csv instead of overwriting "
        "(use this for every recording after the first one)",
    )
    ...
    for seg in segments:
        ...
        filename = f"{args.prefix}_seg_{seg['id']:04d}.wav"
    ...
    write_header = not (args.append and os.path.exists(debug_path))
    with open(debug_path, mode, newline="", encoding="utf-8") as f:
        writer = csv.writer(f, delimiter="|")
        if write_header:
    ...
    print(f"\nKept {kept} clips from this recording ({total_kept_duration:.1f} minutes)")
    print(f"Skipped {too_short} too-short, {too_long} too-long segments")
    print(f"{'Appended to' if args.append else 'Wrote'} {metadata_path} and {debug_path}")
